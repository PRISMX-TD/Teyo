 ...
 *    + 分录全删全插），所以**同一份 payload** 被串起来之后跑两遍与跑一遍
 *    的结果相同——这一段说的就是重复提交（双击、断网重发）这一种。
 *
 *    换成两个人各改各的，串行化只保证不会交错写坏，不保证后一个人看得见
 *    前一个人改了什么：没有版本列可比，后提交的那份整体覆盖先提交的那份，
 *    就是 last-write-wins，先写的那次修改静默丢失。要挡住这个得有真的
 *    乐观并发（版本列 + where 带版本），这里没有。
